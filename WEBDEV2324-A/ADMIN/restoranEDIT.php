...
<?php 
	...
	if(isset($_POST['Ubah']))
	{
		...
		mysqli_query($connection, "update restoran set restoranNAMA='$restorannama', kategoriKODE='$kategorikode' where restoranKODE='$restorankode'");
	}
	$datakategori = mysqli_query($connection, "select * from kategoriwisata");

	//Menerima kirim data dari kategoriwisata.php
	$koderestoran = $_GET['Edit'];
	$editrestoran = mysqli_query($connection, "select * from restoran where restoranKODE = '$koderestoran'");
	$rowedit = mysqli_fetch_array($editrestoran);

	$editanrestoran = mysqli_query($connection, "select * from restoran, kategoriwisata where restoranKODE = '$koderestoran' and restoran.kategoriKODE = kategoriwisata.kategoriKODE");
	$rowedit2 = mysqli_fetch_array($editanrestoran);
?>
...
<div class="col-sm-10">
	<form method="POST" enctype="multipart/form-data">
		<div class="mb-3 row">
			...
			<div class="col-sm-10">
				<input type="text" class="form-control" id="restorankode" name="koderestoran" placeholder="Kode Restoran" maxlength="4" value="<?php echo $rowedit["restoranKODE"]?>" readonly>
			</div>
		</div>
		...
		<div class="mb-3 row">
			...
			<div class="col-sm-10">
				<input type="text" class="form-control" id="restorannama" name="namarestoran" placeholder="Nama Restoran" value="<?php echo $rowedit["restoranNAMA"]?>">
			</div>
		</div>
...
<div class="row">
	...
	<table class="table table-dark table-striped">
		...
		<tbody>
			...
			{
  				$search = $_POST["search"];
  				$query = mysqli_query($connection, "select * from restoran where restoranKODE like '%".$search."%'");
  			} else
  				{
